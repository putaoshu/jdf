...
widget.publish = function(name){
	var severDirname = 'share/jdfwidget/widget';
	...
	ftp.mkdir(severDirname);

	var widgetname = name.replace(/^widget\//, '');
	var param = [0, 0, 0, '-custom', name, severDirname+'/'+widgetname];
	jdf.upload(param, false, function(){
		 console.log('jdf widget "'+widgetname+'" publish success!');
	});
}
