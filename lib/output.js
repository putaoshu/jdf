...
output.init = function(options){
	...
	if(weinre){
		fs.readdirSync(htmlDir).forEach(function(name){
			if(f.isFile(name)){
				debugHtml += '<li style="padding-bottom:0.2em;"><a target="_blank" href="'+mobileWeinre(path.join(htmlDir, name))+'">'+name+'</a></li>';
			}
		});
	...
	var core = function() {
		...
		var copyDefaultDir = function(){
			...
			f.copy(cssDir, cssOutputDir, '(css|'+$.imageFileType()+')$', '(less|scss)$', null, null, null, encoding);
			f.copy(jsDir, jsOutputDir, null, null, null, null, null, encoding);

			// 输出widget todo 可配置
			var outputWidgetDir = outputdir+'/'+jdf.config.widgetDir;
			f.copy(widgetDir, outputWidgetDir,  '(js|css|'+$.imageFileType()+')$', '(less|scss|psd)$', null, null, null, encoding);
		...
		switch (type){
			...
			case 'hashtml': 
				...
				f.copy(htmlDir, outputdir+'/html', null, null, null, null, null, encoding);
				break;
			case 'custom':
				if(!list) return;
				...
				for (var i=0; i<listArray.length; i++ ){
					...
					if ( f.exists(item) ) {
						...
						if (isbackup){ 
							...
							f.copy(source, target, null, '(vm|tpl|less|scss|psd)$', null, null, null, encoding);
							customItemRes+=item+',';
						}
					}
				}
