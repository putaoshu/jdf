...
output.init = function(options){
	...
	var core = function() {
		...
		var copyDefaultDir = function(){
			...
			f.copy(jsDir, jsOutputDir, null, excludeFiles, null, null, null, encoding);

			// 输出widget todo 可配置
			var outputWidgetDir = outputdir+'/'+jdf.config.widgetDir;
			f.copy(widgetDir, outputWidgetDir,  '(js|css|'+$.imageFileType()+')$', (excludeFiles ? excludeFiles : '(less|scss|psd)'), null, null, null, encoding);
