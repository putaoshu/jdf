...
output.init = function(options){
	...
	var core = function() {
		...
		var copyDefaultDir = function(){
			...
			f.copy(cssDir, cssOutputDir, '(css|'+$.imageFileType()+')$', '(less|scss)$');
			f.copy(jsDir, jsOutputDir);

			// 输出widget todo 可配置
			var outputWidgetDir = outputdir+'/'+jdf.config.widgetDir;
			f.copy(widgetDir, outputWidgetDir,  '(js|css|'+$.imageFileType()+')$', '(less|scss|psd)$');
				...
				for (var i=0; i<listArray.length; i++ ){
					...
					if ( f.exists(item) ) {
						...
						if (isbackup){ 
							...
							f.copy(source, target, null, '(vm|tpl|less|scss|psd)$');
							customItemRes+=item+',';
						}
					}
				}
