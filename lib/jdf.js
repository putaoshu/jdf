...
jdf.server = function(autoOpenurl, comboDebug, callback) {
    ...
    FindPort(localServerPort, function(data) {
        if(jdf.config.build.hasBrowserSync){
            ...
            bs.init({
                server: jdf.bgCurrentDir
            });
        }else{
            //default server
            if (!data) {
                console.log('jdf findPort : Port ' + localServerPort + ' has used');
            ...
            console.log('jdf server running at : ');
            console.log(Colors.gray('----------------------------------------'));
            console.log('   Local: '+Colors.magenta('http://localhost:' + localServerPort + '/'));
            console.log('External: '+Colors.magenta('http://'+jdf.getIp()+':' + localServerPort + '/'));
            console.log(Colors.gray('----------------------------------------'));
            console.log('Serving files from: '+jdf.bgCurrentDir);
        }
        ...
        if(jdf.config.build.es6Entry){
            ...
            if(jdf.config.outputCustom) {
                ...
                for (var i=0; i<listArray.length; i++ ){
                    var item = listArray[i];
                    if ( f.exists(item) ) {
                        BuildES6Js.init(item, jdf.bgCurrentDir);
                    }
                }
            }else{
