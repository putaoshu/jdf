...
jdf.init = function(argv){
	...
	jdf.getConfig(function(){
		...
		//extra commands
			...
			//Log.send('server');
			Server.init('./', jdf.config.localServerPort, jdf.config.cdn);
...
jdf.argvInit = function(runType, argv, callback){
	if(runType == 'build' || runType == 'release'){
		...
		jdf.server(autoOpenurl, function(data){
...
*/
jdf.server = function(autoOpenurl, callback){
	...
	FindPort(localServerPort,function(data){
		...
		Server.init(jdf.bgCurrentDir, localServerPort, jdf.config.cdn, jdf.getProjectPath());
...
/**
* @jdf模版生成 demo/x/x.html , jd/ui/x/x.js
