	...
	var cssReplace= function (str,regStr){
		...
		if (regResult){
			...
			if(comboArray.length>0){			
				...
				var staticUrl =  cdnPrefix + comboArray.join(',');
		...
		result.forEach(function(resultItem){
			...
			if (dirExists){
				...
				files.forEach(function(item){
					...
					//$.is.tpl(item)
					if (buildTplTag && (item == widgetName+$.is.tplSuffix || item == widgetName+$.is.vmSuffix) ){
						...
						var dataSourceUrl = fileUrlDirname+widgetName+$.is.dataSourceSuffix ;
							try {
								if (f.exists(dataSourceUrl)) {
									dataSourceContent = f.read(dataSourceUrl);
									if (dataSourceContent)  dataSourceContent = JSON.parse(dataSourceContent);
								}
							} catch (e) {
							...
							try {
								if (widgetData) widgetData = JSON.parse(widgetData);
							} catch (e) {
							...
							//vm处理
							if (dataObj) placeholder = Vm.rander(placeholder, dataObj, fileUrlDirname);
