...
jdf.init = function(argv) {
    ...
    jdf.getConfig(function(configData) {
        ...
        if (argv.length < 3 || cmd2 === '-h' || cmd2 === '--help') {
            ...
            var cpDirPath = jdf.config.cpDirPath || []
            ...
            cpDirPath.forEach(function(item) {
                ...
                if (fs.existsSync(_sourcePath) && fs.existsSync(_targetPath)) {
                    cp(_sourcePath, _targetPath)
                }
            })

            function cp(sourcePath, targetPath) {
                if (sourcePath && targetPath) {
                    console.log('cp -r ' + sourcePath + ' to ' + targetPath)

                    var shelljs = require('shelljs')

                    if (shelljs.cp('-r', sourcePath, targetPath).code !== 0) {
                        console.log('Error: cp failed')
                    } else {
                        console.log('cp success!')
                    }
                } else {
                    console.log('not found sourcePath or targetPath')
                }
            }
        } else {
