...
jdf.init = function(argv){
	...
	jdf.getConfig(function(configData){
		...
		//csslint
		} else if(cmd2 === 'csslint'){
			var cmd3 = argv[3];
			var csslintFiles = (typeof(cmd3) == 'undefined') ? f.currentDir() : cmd3;
			Log.send('csslint');
			Csslint.init(csslintFiles);

		//htmllint
		} else if(cmd2 === 'htmllint'){
			var cmd3 = argv[3];
			var htmllintFiles = (typeof(cmd3) == 'undefined') ? f.currentDir() : cmd3;
			Log.send('htmllint');
			Htmllint.init(htmllintFiles);
