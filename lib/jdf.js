...
jdf.argvInit = function(runType, argv, callback){
	if(runType == 'build' || runType == 'release'){
		...
		f.del(jdf.bgCurrentDir,function(){
			...
			//默认
			var outputType = 'default' ,outputList, isbackup = false;
			...
			jdf.output(outputType, outputList, isbackup, callback);
		});
	}
...
jdf.upload = function(argv, haslog, callback){
	...
	var outputFnWatch = function(){
		...
		jdf.output('debug', null, null, function(){
			 ftpFn(uploadSource, uploadTarget);
		});
	}
...
/**
* @服务器
* @param {Boolse} autoOpenurl 
	true: html/index.html存在的话打开, false时默认打开 http://localhost:3000/
	false: 只启动不打开网页
*/
...
* @param {String} type 'debug' : debug下输出js,css和html文件夹 如$ jdf o -html
* @param {String} type 'custom' : 自定义输出 如$ jdf o app/js/test.js
* @param {String} list  : 自定义输出的文件路径,如app/js/test.js
* @param {Function} callback 回调函数
* @todo 只复制改动的文件
*/
jdf.output = function(type, list, isbackup, callback){
	...
	var core = function(){
		...
		switch (type){
			...
			case 'custom':
				...
				if (!isbackup) { logText = 'jdf output ['+itemRes+'] done!';}
				break;
		}
