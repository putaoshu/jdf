...
jdf.output = function(type, list, callback){
	...
	var core = function(){
		...
		var copyDefaultDir = function(){
			f.copy(jsDir, outputdir+'/'+jdf.config.jsDir);
			f.copy(cssDir, outputdir+'/'+jdf.config.cssDir, null, '(less|scss)$');
			f.copy(widgetDir, outputdir+'/'+jdf.config.widgetDir, '(js|css|svg|tiff?|wbmp|png|bmp|fax|gif|ico|jfif|jpe|jpeg|jpg|cur)$'), '(less|scss)$';
			//todo $.is.imageFile [bug]has less,scss
		}
		switch (type){
			case 'default' :
				copyDefaultDir();
