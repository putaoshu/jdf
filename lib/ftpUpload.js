...
exports.init = function(argv, hasConfig, haslog, callback){
	...
	var ftpFn = function(source, target){
		if(jdf.config.serverType  == "linux" ){
			var fileUpload = require('./fileUpload.js');
			var sourceFilse = f.getdirlist(source, null, '.svn|Thumbs.db|.DS_Store' );

			fileUpload(sourceFilse, jdf.config.serverDir, jdf.config.host, argv, function(){
				jdf.dot.end(haslog);
				if(haslog) console.log('jdf upload ['+jdf.config.host +' | '+ jdf.config.serverDir+'] success!');
	...
	}else if( (argv[3] == '-custom'||argv[3] == '-c') && argv[4] ){
		//custom upload	
		if ( f.exists(argv[4]) ) {
