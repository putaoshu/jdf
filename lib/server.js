		...
		fs.exists(realPath, function (exists) {
			...
			if (isComboUrl) {
				...
				if(f.isFile(realPath)){
                    ...
                    // jsonp格式，自动用callback参数值包裹文件内容
                    if(regCallBack.test(requestUrl)) {
                        strCallBack = regCallBack.exec(requestUrl)[1];
                        content = strCallBack + '(' + content.toString() + ');';
                    }
