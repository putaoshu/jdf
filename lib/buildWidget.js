...
buildWidget.init = function(inputPath,content,type,callback){
	...
	if (result){
		...
		result.forEach(function(resultItem){
			...
			if (widgetType && (widgetType =='tpl'||widgetType =='vm'||widgetType =='css'||widgetType =='js') ) {
				if (widgetType =='tpl') {
					buildJsTag = false;
					buildCssTag = false;
					buildVmTag = false;
			...
			if (dirExists){
				...
				files.forEach(function(item){
					...
					//tpl vm Compile 如果文件夹中vm,tpl同时存在时,只处理vm
					...
					//tpl
					if ( buildTplTag && (item == widgetName+$.is.tplSuffix) ){
						tmplCompileFn();
					}
					
					//vm
					if ( buildVmTag && item == widgetName+$.is.vmSuffix ){
						tmplCompileFn(true);
					}
