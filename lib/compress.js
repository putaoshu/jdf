...
compress.addJsDepends = function(rSource, sourceCompress, sourceOrigin, isdebug){
	...
	var dependenceArray = [];
	var arr = sourceCompress.match(/require\s*\(\s*("|')(.*?)("|')\s*\)/gmi);
	...
	if (arr) {
		for (var i =0; i<arr.length; i++ ){
			...
			if(temp){
				...
				//var a=require('a.js') ==> var a=require('projectPath/a.js')
				source = source.replace(new RegExp(temp[1].replace(/\//g, '\\/'), 'g'), match);
	...
	*/
	if (dependenceArray.length>0) {
		//insert 
		var arrTemp = /(define\(.*?["|'].*?["|']).*?,function/m.exec(source);
...
compress.js = function(source, isdebug){
	...
	try {
		...
		if(jdf.config.output.hasBanner && !isdebug){
			result = compress.setPrefixBanner(source, result) + result + '\r\n';
		}
	}catch (e) {
		if (e && e.message) {
...
compress.css = function(source, isdebug){
	...
	//项目在C盘,build在D盘
	cssDir = cssDir.replace(path.normalize(jdf.config.outputDirName), '');
...
compress.cssImagesUrlReplace = function (source, str, cdn, prefix, suffix) {
    ...
    if (temp) {
		...
		for (var i in tempObj){
			...
			if ( b != 'about:blank' && !$.is.httpLink(b)) {
