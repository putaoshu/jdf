...
compress.css = function(source, isdebug){
	...
	*/
	...
	//增加域名前缀
	if(jdf.config.output.cssImagesUrlReplace && !isdebug) {
		result = compress.cssImagesUrlReplace(result, jdf.config.cdn, cssDir);
	}
...
*/
compress.cssImagesUrlReplace = function (str, cdn, prefix, suffix) {
	if(!jdf.config.cdn){
		return str;
	}
